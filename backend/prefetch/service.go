...
func (s *Service) checkNewDomains() {
	for domain, prefetchDomain := range s.Domains {
		cacheKey := s.buildCacheKey(domain, dns.Type(prefetchDomain.QType))
		if _, exists := s.DNS.Cache.Load(cacheKey); !exists {
...
func (s *Service) processExpiredEntries() {
	...
	s.DNS.Cache.Range(func(key, value interface{}) bool {
...
func (s *Service) handleExpiredKeys(expiredKeys []interface{}) {
	for _, key := range expiredKeys {
		if value, exists := s.DNS.Cache.Load(key); exists {
			if cachedDomain, ok := value.(server.CachedRecord); ok {
				s.DNS.Cache.Delete(key)
